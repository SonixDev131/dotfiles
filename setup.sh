...
update_system() {
  if [[ "$OS" == "ubuntu" ]]; then
    ...
    softwareupdate -l
    echo "💡 Please ensure your macOS is up to date through System Preferences"
    softwareupdate -ia
...
install_homebrew() {
  if ! command -v brew &>/dev/null; then
    ...
    # Add Homebrew to PATH for current session (OS-specific paths)
    if [[ "$OS" == "macos" ]]; then
...
show_usage() {
  ...
  echo ""
  echo "Supported OS: Ubuntu, macOS"
}
...
dry_run() {
  ...
  echo "  • System updates ($OS specific)"
  echo "  • Homebrew package manager"
  echo "  • Rust programming language"
  echo "  • CLI tools: git, fish, starship, eza, bat, fd, ripgrep, lazygit, btop, duf, atuin, tldr, curlie, stow, tmux, fzf, bob"
  echo "  • Personal dotfiles configuration"
  echo "  • Fish shell as default"
  echo ""
  echo "To actually run the setup: $0"
}
